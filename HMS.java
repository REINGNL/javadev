...
public class HMS extends Application {
    ...
    private static void writeErrorLog(Exception a) {
        try {
    ...
    private void mainPage(String staffID, String EmployeeType) {
        ...
        checkoutPage(checkoutLayout, mainPageStage);
    ...
    private static void patientPage(VBox patientLayout) {
        ...
        repopulateTable(table, "", patientLayout);
        ...
        searchBtn.setOnAction(event -> repopulateTable(table, searchField.getText(), patientListBox));

        // Refersh Button
        Button refreshBtn = new Button("Refresh");
        refreshBtn.setFont(patientLabelFont);
        refreshBtn.setOnAction(event -> {
            repopulateTable(table, "", patientListBox);
            searchField.clear();
        });
    ...
    private static Button checkoutPage(VBox checkoutLayout, Stage stage) {
        ...
        resetBtn.setOnAction(event -> {
            ...
            checkoutPage(checkoutLayout, stage);
        });
        //cancel button
...
private static void repopulateTable(TableView<ObservableList<String>> table, String searchInfo, VBox checkoutLayout) {
    try {
        Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/hms", "root", "");
        Statement stmt = con.createStatement();
        ...
        errorAlert.setContentText("Error: " + ex.getMessage());
        errorAlert.showAndWait();
        writeErrorLogs(ex); // Here is where you call the method
        ex.printStackTrace();
    }
    ...
    logoutColumn.setCellFactory(tc -> {
        ...
        logoutButton.setOnAction(e -> {
            ...
            checkoutPage(checkoutLayout, null);
...
private static void writeErrorLogs(Exception ex) {
    try {
        FileWriter fileWriter = new FileWriter("error.log", true); // true to append the log
        PrintWriter printWriter = new PrintWriter(fileWriter);
